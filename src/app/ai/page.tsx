...
export default function AIPage() {
  ...
  return (
    <div className="fixed inset-0 flex flex-col lg:relative lg:inset-auto lg:h-screen" style={{ backgroundColor: '#060E1A' }}>
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        ...
        <div className="absolute left-1/4 rounded-full" style={{ bottom: showIntro ? '32%' : '22%', width: '6px', height: '6px', backgroundColor: 'rgba(0,240,210,0.9)', boxShadow: '0 0 12px 4px rgba(0,240,210,0.4)', transform: 'translateY(50%)' }} />
        <div className="absolute left-3/4 rounded-full" style={{ bottom: showIntro ? '32%' : '22%', width: '4px', height: '4px', backgroundColor: 'rgba(0,220,190,0.7)', boxShadow: '0 0 8px 3px rgba(0,220,190,0.3)', transform: 'translateY(50%)' }} />
      </div>

      <div className="relative z-10 flex items-center gap-3 px-5 pt-5 pb-3 flex-shrink-0">
        <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ backgroundColor: 'rgba(0,180,160,0.2)', border: '1px solid rgba(0,220,190,0.3)' }}>
        ...
        {messages.length > 0 && (
          <button onClick={handleReset} className="ml-auto text-xs px-3 py-1.5 rounded-full" style={{ color: 'rgba(0,220,190,0.8)', backgroundColor: 'rgba(0,220,190,0.08)', border: '1px solid rgba(0,220,190,0.2)' }}>↺ New</button>
        )}
      </div>
      ...
      <div className="relative z-10 flex-1 overflow-y-auto px-5">
        {showIntro && (
          <div className="flex flex-col items-center justify-center min-h-[50vh] text-center">
            <p className="text-xs font-semibold mb-2 tracking-widest uppercase" style={{ color: 'rgba(0,220,190,0.6)' }}>Today feels {daily.mood}</p>
            <h1 className="text-3xl font-bold mb-8 leading-tight" style={{ color: '#fff' }}>What would you<br />like to explore?</h1>
          </div>
        )}
        {messages.length > 0 && (
          <div className="space-y-4 py-4">
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                {msg.role === 'assistant' && (
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center mr-2 flex-shrink-0 self-end" style={{ backgroundColor: 'rgba(0,180,160,0.2)', border: '1px solid rgba(0,220,190,0.3)' }}>
                ...
                <div className="max-w-[80%] px-4 py-3 text-sm leading-relaxed whitespace-pre-line"
                  style={{ backgroundColor: msg.role === 'user' ? 'rgba(255,255,255,0.08)' : 'rgba(0,180,160,0.08)', color: msg.role === 'user' ? 'rgba(255,255,255,0.9)' : 'rgba(220,255,250,0.95)', border: msg.role === 'user' ? '1px solid rgba(255,255,255,0.1)' : '1px solid rgba(0,220,190,0.2)', borderRadius: msg.role === 'user' ? '18px 18px 4px 18px' : '18px 18px 18px 4px' }}>
                  {msg.text}
                </div>
              </div>
            ))}
            {loading && (
              <div className="flex justify-start items-end gap-2">
                <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: 'rgba(0,180,160,0.2)', border: '1px solid rgba(0,220,190,0.3)' }}>
                  <span style={{ color: 'rgba(0,220,190,1)', fontSize: '0.8rem' }}>◈</span>
                </div>
                <div className="px-4 py-3 rounded-2xl text-sm" style={{ backgroundColor: 'rgba(0,180,160,0.08)', border: '1px solid rgba(0,220,190,0.2)', color: 'rgba(0,220,190,0.7)' }}>
      ...
      {showIntro && (
        <div className="relative z-10 mx-5 mb-3 flex-shrink-0">
          <div className="px-5 py-4 rounded-2xl text-sm leading-relaxed" style={{ backgroundColor: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.7)', backdropFilter: 'blur(12px)', whiteSpace: 'pre-line' }}>
          ...
          <div className="flex flex-wrap gap-2">
            {chips.map(chip => (
              <button key={chip.label} onClick={() => handleSend(chip.prompt)} disabled={loading}
                ...
                style={{ backgroundColor: 'rgba(0,220,190,0.08)', border: '1px solid rgba(0,220,190,0.25)', color: 'rgba(0,220,190,0.9)', backdropFilter: 'blur(8px)', opacity: loading ? 0.4 : 1 }}>
                {chip.label}
              </button>
            ))}
          </div>
      ...
      <div className="relative z-10 px-5 pb-6 pt-1 flex-shrink-0">
        <div className="flex gap-2 items-center px-4 py-3 rounded-2xl" style={{ backgroundColor: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', backdropFilter: 'blur(12px)' }}>
          <input type="text" value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()}
            placeholder="Write something..." className="flex-1 text-sm bg-transparent focus:outline-none"
            style={{ color: 'rgba(255,255,255,0.85)', caretColor: 'rgba(0,220,190,1)' }} />
          <button onClick={() => handleSend()} disabled={loading || !input.trim()} className="text-sm font-semibold transition-all"
            style={{ color: loading || !input.trim() ? 'rgba(255,255,255,0.25)' : 'rgba(0,220,190,1)' }}>Send</button>
        </div>
      </div>
    </div>
  );
